...
module {
    ...
    public class Tokens(feeTo_: Principal, tokenInfoList: [(Text, TokenInfo)]) {
        ...
        public func getMetadata(tokenId: Text): TokenAnalyticsInfo {
            switch(tokens.get(tokenId)) {
                case(?token) {
                    return _toTokenAnalyticsInfo(token);
                };
                case(_) { return _toTokenAnalyticsInfoEmpty(tokenId); };
            };
        };

        public func getSymbol(tokenId : Text) : Text {
            switch (tokens.get(tokenId)) {
                case(?token) {
                    token.symbol
                };
        ...
        private func _toTokenAnalyticsInfo(info: TokenInfo): TokenAnalyticsInfo {
            {
                name = info.name;
                symbol = info.symbol;
                decimals = info.decimals;
        ...
        private func _toTokenAnalyticsInfoEmpty(tokenId: Text): TokenAnalyticsInfo {
            {
                name = tokenId;
                symbol = "";
                decimals = 0;
